    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
        ...
        <Pressable onPress={onDismiss} hitSlop={8}>
          <MaterialIcons name="close" size={20} color="#555577" />
        </Pressable>
      </View>

      <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 6, gap: 8, flexWrap: 'wrap' }}>
        <View style={{ backgroundColor: `${threatColor}22`, borderRadius: 99, paddingHorizontal: 8, paddingVertical: 2 }}>
          <ThemedText size="xs" style={{ color: threatColor }}>
            {getThreatEmoji(sighting.threatLevel)} {getThreatLabel(sighting.threatLevel)}
          </ThemedText>
        </View>
        {sighting.species && (
          <ThemedText variant="muted" size="xs">· {capitalize(sighting.species)}</ThemedText>
        )}
      ...
      >
        <ThemedText weight="semibold" size="sm" style={{ color: '#0A0A1A' }}>
          View Details →
        </ThemedText>
      </Pressable>
    </View>
