import { deleteSighting, getSightings, insertSighting, updateSighting } from '@/lib/storage';
...
export function useSightingsDB() {
  ...
  const loadSightings = useCallback(async (filter?: { threatLevel?: ThreatLevel }) => {
  ...
  const addSighting = useCallback(async (sighting: Omit<Sighting, 'id'>): Promise<Sighting> => {
  ...
  const editSighting = useCallback(
    async (id: string, updates: Partial<Omit<Sighting, 'id'>>): Promise<void> => {
      ...
      setSightings((prev) => prev.map((s) => (s.id === id ? { ...s, ...updates } : s)));
    },
    []
  );
